...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate WorldSimProbe Task 3.")
    ...
    with tempfile.TemporaryDirectory(prefix="worldsimprobe_task3_") as temporary:
        manifest = Path(temporary) / "joined.jsonl"
        write_jsonl(manifest, rows)
        result = evaluate_task3_manifest_robotseg_flow(
            manifest,
            args.reference_root,
